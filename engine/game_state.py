"""IRON PRICE: Kingsmoot — Central Game State Manager"""
import random
...
class GameStateManager:
    ...
    def _setup_initial_ships(self):
        ...
        # Victarion at Great Wyk
        vic_flagship = Ship(id="victarion_flagship", faction="Victarion", is_flagship=True, crew=4, max_crew=4)
    ...
    def action_sail(self, ship_id: str, target_node_id: str) -> Dict[str, Any]:
        """Sail a ship up to speed 2 to an adjacent or reachable node."""
        if self.game_over:
        ...
        # Check reachability within max speed (2)
        reachable = self.map_engine.get_reachable_nodes(curr_node_id, max_speed=2)
        if target_node_id not in reachable:
            return {"success": False, "error": f"Node {target_node.name} is not reachable from {curr_node_id} (speed 2)."}

        # Move ship
        curr_node = self.nodes[curr_node_id]
        curr_node.occupants.remove(ship)
        target_node.occupants.append(ship)
    ...
    def action_muster(self, node_id: str, ship_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Must be controlled by player or player's home node
        ...
        # Cost: 2 at Great Wyk, 3 elsewhere
        ...
        # Check total capacity on friendly ships at this node
        ...
        # 1. Fill target ship first if it has capacity
        ...
        # 2. Overflow to any other friendly ships at this port with capacity
        if crew_to_add > 0:
            for s in friendly_ships:
        ...
        # 3. Any remainder goes to reserve crew
        if crew_to_add > 0:
    ...
    def _check_auto_turn_advance(self):
        """Auto advance if no actions remaining."""
        if self.actions_remaining <= 0:
            self._advance_turn()

    def _advance_turn(self):
        """Advance player turn, round, and season."""
    ...
    def _resolve_season_end(self):
        ...
        # Control scoring: +1 Legend per occupied Isle
    ...
    def _resolve_game_over(self):
        ...
        # Determine winner by Legend (tie-break: Hoard -> Favor)
