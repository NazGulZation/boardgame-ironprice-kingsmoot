...
8. **Naval Clash Animation & Popup Synchronization**:
   ...
   * AI claimant raids are map-only: the reave dice popup is suppressed during AI turns (it would cover the axe-flight animation); the bottom dice tray shows the tumbling dice via `renderDiceRoll(..., true)` re-applied after `refresh()`. `stepAi` is guarded by `_steppingAi` against overlapping invocations.
    * Combat-before-respawn staging: the backend records wiped hulls (`BattleState.sunk_ship_ids` + `sunk_ships` snapshots, `ReaveOutcome.dead_ship_ids` + `dead_ships`) *before* WHAT IS DEAD moves them to limbo. The frontend re-stages limbo hulls from snapshots at the fight site (`_injectSunkenHulls` in `playShipSinking` / `raidTableau`), plays swords first, awaits the sinking animation, and only then restores live state — the hull stays visible until the sinking completes. `_battleTableau` also merges reinforcing arrivals from nextState so sailed ships never snap back to origin mid-clash. Home-harbor recovery publishes `last_harbor_recovery` in state; `MapRenderer.update` plays the muster-style `+N` crew-gain floater exactly once per turn (seen-key guarded).
 ...
 9. **Tactical Fleet Stacking & Respawn Mechanics**:
    ...
    * **What Is Dead May Never Die**: When any ship's crew is reduced to 0, it leaves the board into off-board limbo and returns at its owner's next turn start at its home port with **1 crew** (if Flagship) or **0 crew** (if standard Longship). No faction is ever eliminated. Backend records `sunk_ship_ids` / `dead_ship_ids` BEFORE limbo intake so the UI can sink hulls at the fight site first.
 ...
 10. **Settlement Garrison Attrition & Harbor Recovery**:
    ...
    * **Home Harbor Crew Recovery**: From turn 2 onward (the opening round is recovery-free so the first player opens on equal footing), at the start of its turn ONE faction ship already docked at its OWN home node gains $+1$ crew — flagship first, else the highest-crew hull, capped at max crew. Reavers are eligible when the flagship needs nothing. Ships washing ashore from limbo the same turn start are excluded until their following turn, keeping the 1/0 return loadout exact.
...
* **Phase 2: Combat & Favor** (Completed)
  ...
  * Implemented: Storm Belt hazards (Black Wind only immune), deferred no-elimination respawn ("What is dead may never die": sunk hulls wait in limbo, return 1 crew flagship / 0 crew reaver at owner's next turn), southern map re-routing (`bay–storm–seaS` chain, `seaS`→fair/banefort/flint), garrison attrition + seasonal replenishment, home-node harbor recovery, and persistent logging subsystem (`logs/game.log`, `logs/error.log`).
